...
export async function POST(request) {
  try {
    ...
    if (gorsel && gorsel.size > 0) {
      ...
      if (yuklemeHatasi) {
        return NextResponse.json({ hata: "YUKLEME: " + yuklemeHatasi.message }, { status: 500 });
      }
    ...
    if (error) {
      return NextResponse.json({ hata: "EKLEME: " + error.message }, { status: 500 });
    }

    return NextResponse.json({ basarili: true });
  } catch (e) {
    return NextResponse.json({ hata: "GENEL HATA: " + e.message }, { status: 500 });
  }
}